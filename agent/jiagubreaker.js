...
const dlopen = new NativeFunction(libc.getExportByName('dlopen'), 'pointer', ['pointer', 'int'])
const dlsym = new NativeFunction(libc.getExportByName('dlsym'), 'pointer', ['pointer', 'pointer'])
...
function hookdex(libart) {
    const symbol = '_ZN3art11ClassLinker11DefineClassEPNS_6ThreadEPKcmNS_6HandleINS_6mirror11ClassLoaderEEERKNS_7DexFileERKNS_3dex8ClassDefE'
    ...
    Interceptor.attach(dex, {
        onEnter(args) {
            ...
            dexes.set(`${base}`, size)
            console.log('[dex]', `${base.readCString(4).replace('\n', '\\n')} ${size}`)
...
function hookhide(linker, predicate) {
    const solist_get_head = new NativeFunction(getEnumerated(linker, '__dl__Z15solist_get_headv'), 'pointer', []);
    const soinfo_get_soname = new NativeFunction(getEnumerated(linker, '__dl__ZNK6soinfo10get_sonameEv'), 'pointer', ['pointer']);
    const soinfo_get_realpath = new NativeFunction(getEnumerated(linker, '__dl__ZNK6soinfo12get_realpathEv'), 'pointer', ['pointer']);
