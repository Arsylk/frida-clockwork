...
import { Libc, Consts, tryNull } from '@clockwork/common';
...
function asExportedObject(module: Module) {
  return Object.fromEntries(module.enumerateExports().map(({ name, address }) => [name, address]));
}
...
function readFdPath(fd: number, bufsize: number = Consts.PATH_MAX): string | null {
  const strpath = `/proc/${Process.id}/fd/${fd}`;
  // return strpath;
  const buf = Memory.alloc(bufsize);
  const path = Memory.allocUtf8String(strpath);

  const _ = Libc.syscall_readlinkat(78, 0, path, buf, bufsize);
