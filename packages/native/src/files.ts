...
function hookOpen(
  ...
  function log(
    ...
    logger.info({ tag: key }, `${struri} flags: ${flagsEnum}, ${mode ? `mode: ${mode} ${errstr}` : ''} ${m}`);
  }
  // Interceptor.replace(
  //     Libc.open,
  //     new NativeCallback(
    ...
    new NativeCallback(
      function (pathname) {
        ...
        if (predicate(this.returnAddress)) {
          ...
          const msg = ofResultColor(replaceStr ? `${pathnameStr} -> ${replaceStr}` : `${pathnameStr}`, ret);
...
function hookRemove(predicate: (ptr: NativePointer) => boolean, fn: (path: string) => boolean = () => true) {
      ...
      new NativeCallback(
        function (pathname) {
          if (predicate(this.returnAddress)) {
            ...
            const replace = fn(strpath);
            const fmt = replace ? green : red;
            logger.info({ tag: key }, `${fmt(bold(gray(strpath)))} ${addressOf(this.returnAddress)}`);
            if (replace) return 0;
          }
