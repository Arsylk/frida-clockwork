      ...
      switch (data[key]) {
        case 'char':
          cache[key] = {
            ptr: crnt,
            get value() {
              return String.fromCharCode(Number(crnt) & 0xffff);
            },
          };
          offset += 0x1;
          break;
        ...
        case 'string':
          cache[key] = {
            ptr: crnt,
            get value() {
              return !isNully(crnt) ? crnt.readCString() : null;
            },
          };
        ...
        case 'string*':
          cache[key] = {
            ptr: crnt,
            get value() {
              if (isNully(crnt)) return null;
              const read = crnt.readPointer();
              if (isNully(read)) return null;
              return read.readCString();
            },
          };
          offset += Process.pointerSize;
          break;
