...
function hookLibart(predicate: (thisRef: InvocationContext | CallbackContext) => boolean, full: boolean) {
  ...
  const GetFieldText = (
    ...
    const typeName = signatureToPrettyTypes(sigName)?.[1] ?? sigName;
      ...
      onEnter({ method, env, methodID, jArgs }) {
        ...
        switch (method?.className) {
          case 'org.godotengine.godot.Godot':
            ...
            break;
          case 'com.unity4d.player.UnityPlayer':
...
function barebone(
  ...
  repl(envWrapper, JNI.RegisterNatives, function (retval, env, clazz, jMethodDef, count) {
    ...
    for (let i = 1; i < count; i++) {
      const namePtr = jMethodDef.add(i * Process.pointerSize * 4).readPointer();
      const sigPtr = jMethodDef.add(i * Process.pointerSize * 4 + Process.pointerSize).readPointer();
      const fnPtrPtr = jMethodDef.add(i * Process.pointerSize * 4 + Process.pointerSize * 2).readPointer();
