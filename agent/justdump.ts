...
const clone = Module.getGlobalExportByName('clone');
Interceptor.attach(clone, {
    onEnter: (args) => {
        if (!isNully(args[3])) {
            const addr = args[3].add(96).readPointer();
            const so_name = Process.getModuleByAddress(addr).name;
            const so_base = Process.getModuleByName(so_name).base;
...
Interceptor.attach(Module.getGlobalExportByName('android_dlopen_ext'), {
    ...
    onLeave: function (retval) {
        if (this.match) {
            ...
            const base = Process.getModuleByName('libDexHelper.so').base;
